# PillarHist × OpenPCDet 复刻与部署指导规范 v1.3

> 生效日期：2026-09-17  
> 适用工程：`/home/ksyang/OpenPCDet`  
> 固定基线提交：`e90de060bceba2a228095eea01718b4ded55b68b`（分支 `pointpillars-pillarhist`）  
> 当前目标：在不改变 R0–R5 数学语义的前提下完成 R6 optimized path，并以受控短程训练完成 R7 结构筛选。  
> 本文件是 R6–R7 实现、验证和验收的唯一规范入口。v1.0–v1.2、各版评审与审计依据版均只用于追溯。
...
R0–R5 已由提交 `e90de060bceba2a228095eea01718b4ded55b68b` 和运行 `20260917T012049Z_seed666` 关闭。当前阶段完成的定义是：[已冻结][合理推断]

> PH_OPT 通过 REF/OPT 全链路与梯度对齐并获得客观性能结论；四个 PillarHist 候选在完全一致的短程协议下完成结构裁决，输出 `SELECTED`、`INCONCLUSIVE` 或 `NO_PROMOTION`。
...
本轮实施止于 R7。R7 完成后必须人工评审，不自动进入 R8/R9。[已冻结][合理推断]
...
| ENV-002 | [已冻结][原文明示] 本地检查 | R6–R7 入口 Git HEAD `e90de060bceba2a228095eea01718b4ded55b68b`；分支 `pointpillars-pillarhist` |
...
**v1.3 当前验收结论：** R6 只有在 TST-018 与全链路/梯度对齐通过后才能发布性能结论；R7 必须以预注册初始化、数据重放和短程协议输出 `SELECTED`、`INCONCLUSIVE`、`NO_PROMOTION` 或 `BLOCKED`。本轮结果不等同于完整训练、统计显著性、PTQ 或硬件部署结论。
